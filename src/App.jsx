...
const TRANSLATIONS = {
  it: {
    ...
    alertImported: "✓ Allenamento importato con successo!",
    alertErrorRead: "Errore nella lettura del file. Formato non valido."
  },
  en: {
    ...
    alertImported: "✓ Workout imported successfully!",
    alertErrorRead: "Error reading file. Invalid format."
  }
};
...
function FloatingTimer({ duration, onCancel, triggerKey }) {
  ...
  const showIOSWarning = () => {
    if (isIOS && !localStorage.getItem('gl_ios_timer_warned')) {
      ...
      alert(userLang === 'it' 
        ? '⚠️ Su iPhone l\'avviso sonoro non suona se lo schermo si spegne.\nIl conteggio resta comunque corretto al rientro nell\'app.' 
        : '⚠️ On iPhone, the sound alert won\'t play if the screen turns off.\nThe countdown stays accurate when you return to the app.');
    }
  };
  ...
  const playCompletionFeedback = () => {
    ...
    // Vibrazione (Android; ignorata silenziosamente dove non supportata)
    try { if (navigator.vibrate) navigator.vibrate([200, 100, 200]); } catch (e) {}
  };

  useEffect(() => {
    // Il countdown è derivato da un timestamp assoluto: anche se il browser
    // throttla i timer in background, al rientro il valore resta corretto.
    ...
    const onVisibility = () => {
      if (document.visibilityState === 'visible') {
        tick(); // risincronizza subito il display al rientro
        // Il browser rilascia il wake lock quando la pagina va in background:
        // va ri-acquisito se il timer è ancora attivo.
        if (!doneRef.current) requestWakeLock();
      }
    };
